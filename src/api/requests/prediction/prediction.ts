...
class PredictionService {
	...
	public updatedPortfolio(
		id: number,
		dto: UpdatePortfolioDto,
		config?: AxiosRequestConfig,
	) {
		const normalizedUri = URLS.PREDICTION.UPDATE_PORTFOLIO.replace(
			'$1',
			id.toString(10),
		);
	...
	public getMoexIndexesByPeriod(
		dto: GetMoexIndexesByPeriodDto,
		config?: AxiosRequestConfig,
	) {
		return api.put<MoexIndex[]>(
			URLS.PREDICTION.GET_MOEX_INDEXES_BY_PERIOD,
			dto,
			config,
		);
	}
